  ...
  fetch("https://jsonplaceholder.typeicode.com/photos")
    ...
    .catch((err) => {
      reply
        .code(404)
        .header("Content-Type", "application/json; charset=utf-8")
        .send({ error: "Photos not found", statusCode: 404 });
    });
});
...
fastify.get("/photos/:id", (request, reply) => {
  ...
  fetch("https://jsonplaceholder.typeicode.com/photos/" + id)
    .then((response) => {
      return response.json();
    })
    .then((photo) => {
      reply
        .code(200)
        .header("Content-Type", "application/json; charset=utf-8")
        .send({ error: "", statusCode: 200, photos: photo });
    })
    .catch((err) => {
      reply
        .code(404)
        .header("Content-Type", "application/json; charset=utf-8")
        .send({ error: "ERROR: not found", statusCode: 404 });
    });
});
